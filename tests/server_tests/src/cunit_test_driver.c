...
int		main()
{
  CU_pSuite	server_networking_suite;
  int		r;

  r = 0;
  check(CUE_SUCCESS == CU_initialize_registry(), "init cunit registry");
  server_networking_suite = CU_add_suite("Server Networking suite", initialise_suite, cleanup_suite);
  check_mem(server_networking_suite);
  check(CU_add_test(server_networking_suite, "test naive parser", test_naive_parser) /* && */
	/* CU_add_test(suite, "test naive parser", test_event_loop) */,
	"add test suite");
